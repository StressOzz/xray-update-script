...
if [ "$CURRENT" == "$LATEST" ]; then
    ...
    NEW_VER=$(docker --version 2>/dev/null | awk '{print $3}' | sed 's/,//' | sed -E 's/^[0-9]+:([0-9.]+).*/\1/')
...
# Последняя версия Xray с GitHub
...
if [ "$CURRENT_X" == "$LATEST_X" ]; then
    ...
    NEW_X=$(docker exec $CONTAINER xray --version 2>/dev/null | head -n1 | awk '{print $2}')
